...
class ExportPrestashopProducts(models.TransientModel):
                    ...
                    if prestashop and need_update_id:
                        ...
                        if temp_obj:
                            ...
                            context.update({'price': price, 'reference': default_code,
                                            'weight': weight, 'cost': cost, 'def_categ': def_categ, 'temp_obj':temp_obj})
                            update = self.with_context(context).update_template(
                                prestashop, erp_id, presta_id, name, description, description_sale)
                            if update[0]:
                                temp_obj.channel_mapping_ids.with_context({'search_default_filter_by_channel_id':channel_id.id}).write({'need_sync':'no'})
                                self.env.cr.commit()
        ...
        if product_data.product_tmpl_id:
            ...
            if product_data.attribute_line_ids:
                response_combination = self.create_combination(
                    prestashop, combination_bs, ps_template_id, product_data.product_tmpl_id, erp_product_id, default_attr)
    ...
    def export_template(self, prestashop, product_bs, erp_template_id):
        ...
        product_bs['product'].update({
            'price': str(round(template_data.with_context(pricelist= channel_id.pricelist_name.id).price,2)),
            'active': '1',
    ...
    def create_combination(self, prestashop, add_comb, presta_main_product_id, erp_template_id, erp_product_id, default_attr):
        ...
        price_extra = round(float(obj_pro.with_context(pricelist=channel_id.pricelist_name.id).lst_price) - float(obj_pro.with_context(pricelist=channel_id.pricelist_name.id).list_price), 2)
        ...
        if returnid:
            ...
            try:
                ...
                pass
            # _logger.info('...... Error combination image data:%r ... ', [returnid, data])
            if default_attr:
                try:
            ...
            temp = channel_id.create_product_mapping(
                erp_template_id, obj_pro, presta_main_product_id, pid, {'default_code': default_code, 'barcode': '', 'operation':'export'})
    ...
    def create_normal_product(self, prestashop, erp_template_id, erp_product_id, prest_main_product_id):
        ...
        if add_data:
            add_data['product'].update({
                'price': str(round(product_data.with_context(pricelist= channel_id.pricelist_name.id).price, 2)),
                'active': '1',
                'redirect_type': '404',
                'minimal_quantity': '1',
                'available_for_order': '1',
            ...
            if returnid:
                temp = channel_id.create_product_mapping(
                    erp_template_id, product_data, prest_main_product_id, 'No Variants', {'default_code': default_code, 'barcode': '', 'operation': 'export'})
    ...
    def update_template(self, prestashop, erp_id, presta_id, name, description, description_sale):
        ...
        if isinstance(template_data, dict):
            ...
            if returnid:
                ...
                map_id.with_context({'search_default_filter_by_channel_id':self.channel_id.id}).write({'need_sync':'no'})
    ...
    def update_template_category(self, prestashop, erp_id, presta_id):
        ...
        if search_cat:
            ...
            resp = self.env['export.prestashop.categories'].with_context({'err_msg':'', 'mapping_ids':[], 'create_ids': [], 'channel_id':self.channel_id}).sync_categories(
        ...
        if extra_categ:
            for categ in extra_categ.extra_category_ids:
                cat_id = self.env['export.prestashop.categories'].with_context({'err_msg':'', 'mapping_ids':[], 'create_ids': [], 'channel_id':self.channel_id}).sync_categories(
