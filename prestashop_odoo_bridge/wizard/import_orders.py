...
class ImportOrders(models.TransientModel):
    ...
    def _fetch_prestashop_orders(self, prestashop):
        ...
        date_add = fields.Datetime.to_string(self.channel_id.import_order_date)
        if date_add:
            try:
    ...
    def get_order_vals(self, prestashop, order_id):
        ...
        if res.get('data'):
            ...
            if cust_data['data']:
                ...
                carrier_name = self._get_data(prestashop, 'carriers', shipping_method)
                carrier_name = carrier_name.get('data')['carrier'].get('name')
